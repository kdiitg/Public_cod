...
'''
- **Windows:** Open a command prompt or terminal window. 
Set the `BOT_TOKEN` environment variable with your actual bot token:

  ```bash
  setx BOT_TOKEN YOUR_ACTUAL_BOT_TOKEN
  ```

- **Linux/macOS:** Open a terminal window. Set the `BOT_TOKEN` environment variable:
...
class Linux:
    ...
    _base_url = f"https://api.telegram.org/bot{os.getenv('BOT_TOKEN_LINUX')}" # bot token is comming from env variable
...
if __name__ == '__main__':
    # Start the main loop
    chat_id = 2143751892    # this id is wrong u can use id
    Linux(chat_id).main()
